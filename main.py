...
class ControlsWindow(): 
    ...
    def main_window(self):
        ...
        region1_button = Button(self.root, text="Select region 1", command=self.select_region(region_name='region1'))
        region1_button.pack(in_=self.bottom, side='left')

        region2_button = Button(self.root, text="Select region 2", command=self.select_region(region_name='region2'))
...
def mainopencv(region_name: str="region1"):
    ...
    while(1):
        if (img-imgbase).any():
            ...
            img_ = imgbase.copy()
        img_ = cv.putText(img_, region_name, (10, 50), cv.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2, cv.LINE_AA, False)   
